...
const UNHEALTHY_REPORT: DiagnosticsReport = {
  ...
  issues: [
    ...
    {
      severity: "error",
      file: ".observatory/workflows/generate-video.json",
...
describe("DiagnosticsPanel", () => {
  ...
  it("groups issues by file and renders the path, message, and hint of every issue", () => {
    ...
    expect(screen.getByText(".observatory/workflows/checkout.json")).toBeInTheDocument();
    expect(screen.getByText(".observatory/workflows/generate-video.json")).toBeInTheDocument();
  ...
  it("sorts errors before warnings within a file group, and files with errors before files with only warnings", () => {
    ...
    const fileHeadings = screen.getAllByText(/\.observatory\/workflows\//).map((node) => node.textContent);
    expect(fileHeadings).toEqual([".observatory/workflows/checkout.json", ".observatory/workflows/generate-video.json"]);
  });
  ...
  it("orders file groups with errors before file groups that only have warnings", () => {
    const report: DiagnosticsReport = {
      ...
      issues: [
        {
          severity: "warning",
          file: ".observatory/workflows/aaa-warning-only.json",
          message: "This workflow has more than 14 steps; prefer 5-9 top-level steps.",
        },
        {
          severity: "error",
          file: ".observatory/workflows/zzz-has-error.json",
    ...
    expect(fileHeadings).toEqual([".observatory/workflows/zzz-has-error.json", ".observatory/workflows/aaa-warning-only.json"]);
  });
