/**
 * Private, disposable temp directories for tests that mutate `.observatory/*`. Never touches
...
function uniqueDir(label: string): string {
  return path.join(os.tmpdir(), `code-observatory-e2e-${label}-${randomBytes(4).toString("hex")}`);
}
...
/** A brand-new, empty directory (no `.observatory` at all) for uninitialized-state / CLI tests. */
