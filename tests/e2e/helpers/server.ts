 ...
 * `code-observatory open`, never an in-process fake.
 *
...
async function waitForReady(
  ...
  while (Date.now() < deadline) {
    if (child.exitCode !== null) {
      throw new Error(`code-observatory open exited early (code ${child.exitCode}) before it became ready.\n${describeFailure()}`);
    }
    try {
...
/** Starts `code-observatory open --no-open --port <port> --root <root>` and waits for it to serve `/api/state`. */
export async function startObservatoryServer(root: string, port: number): Promise<ManagedServer> {
