/**
 * `code-observatory init` — scaffolds `.observatory/` (contract §9, product brief §C).
 *
...
const GITIGNORE_ENTRY = ".observatory/.runtime/";
...
function gitignoreAlreadyCoversRuntime(content: string): boolean {
  ...
  return content.split(/\r\n|\r|\n/).some((line) => {
    ...
    return normalized === target || normalized === ".observatory";
  });
}

/**
 * Appends `.observatory/.runtime/` to the repository `.gitignore`, creating it if absent,
...
/** Scaffolds `.observatory/` at the resolved repository root. Never throws; never exits. */
export async function runInit(options: InitOptions): Promise<InitResult> {
  ...
  // .observatory/workflows/ is a directory, not a versioned file: `mkdir recursive` above
...
export function printInitResult(result: InitResult): void {
  ...
  console.log("Code Observatory initialized.");
  console.log("");
  console.log("Created:");
  ...
  console.log("");
  console.log("Next:");
  console.log("code-observatory open");
}
